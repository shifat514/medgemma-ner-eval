"""BC5CDR adapter using bigbio/bc5cdr (bigbio_kb config).
...
class BC5CDRAdapter(BaseNERAdapter):
    def load(self) -> DatasetDict:
        # bigbio/bc5cdr requires the `bioc` package; bc5cdr_bigbio_kb is the
        # only available bigbio config (there is no bc5cdr_bigbio_ner).
        raw = load_dataset(
            "bigbio/bc5cdr", name="bc5cdr_bigbio_kb", trust_remote_code=True
        )
